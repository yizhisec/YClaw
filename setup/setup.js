...
(function () {
  ...
  const I18N = {
    en: {
      title: "YClaw Setup",
      "welcome.title": "Welcome to YClaw",
      "welcome.subtitle": "YClaw is a one-click installer for OpenClaw. Ready to use out of the box.",
      ...
      "config.keyNotice": "YClaw does not provide API keys. Please click the link to purchase one from the provider's website.",
      ...
      "done.title": "All Set!",
      "done.subtitle": "YClaw is ready. Here's what you can do:",
      ...
      "done.start": "Start YClaw",
      "done.starting": "Starting Gateway…",
      "done.startFailed": "Gateway failed to start. Please click Start YClaw to retry.",
      "conflict.title": "Existing OpenClaw Detected",
      "conflict.subtitle": "An existing OpenClaw installation was found on your system. This may cause port conflicts with YClaw.",
    ...
    zh: {
      title: "YClaw 安装引导",
      "welcome.title": "欢迎使用 YClaw",
      "welcome.subtitle": "YClaw 是 OpenClaw 的一键安装包，开箱即用。",
      ...
      "config.keyNotice": "YClaw 不提供 API 密钥，请点击链接前往服务商官网购买 API 密钥后使用。",
      "config.platform": "平台",
      "config.baseUrl": "接口地址",
      ...
      "done.startFailed": 'Gateway 启动失败，请点击"启动 YClaw"重试。',
      "conflict.title": "检测到已安装的 OpenClaw",
      "conflict.subtitle": "系统中已存在 OpenClaw 安装，可能与 YClaw 产生端口冲突。",
  ...
  async function checkExistingInstallation() {
    if (!window.aivoclaw?.detectInstallation) {
      goToStep(1);
      return;
    }
    try {
      const res = await window.aivoclaw.detectInstallation();
  ...
  async function handleUninstall() {
    ...
    try {
      const res = await window.aivoclaw.resolveConflict({
        action: "uninstall",
  ...
  async function handleVerify() {
    ...
    try {
      const result = await window.aivoclaw.verifyKey(params);
      ...
      await window.aivoclaw.saveConfig(buildSavePayload(params));
      setVerifying(false);
      goToStep(3);
    } catch (err) {
  ...
  async function handleComplete() {
    ...
    try {
      ...
      const result = await window.aivoclaw.completeSetup(payload);
  ...
  async function loadLaunchAtLoginState() {
    if (!window.aivoclaw?.setupGetLaunchAtLogin) {
      return;
    }
    try {
      const result = await window.aivoclaw.setupGetLaunchAtLogin();
  ...
  function bindEvents() {
    ...
    els.platformLink.addEventListener("click", (e) => {
      ...
      if (url && window.aivoclaw?.openExternal) {
