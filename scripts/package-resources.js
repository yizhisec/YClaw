...
// 额外技能注入：将此目录下所有含 SKILL.md 的子文件夹复制到网关的 skills/ 中
const EXTRA_SKILLS_DIR = path.join(ROOT, "extraSkills");
...
function extractPosix(tarPath, runtimeDir, version, platform, arch, targetId) {
  ...
  // Windows 宿主交叉打包 macOS 时，优先使用系统自带 bsdtar
  // 因为 Git 自带的 GNU tar 不能正确处理 macOS tar.gz 中的符号链接
...
function buildAnalyticsConfig() {
  const captureURL = readEnvText("AIVOCLAW_ANALYTICS_CAPTURE_URL");
  const captureFallbackURL = readEnvText("AIVOCLAW_ANALYTICS_CAPTURE_FALLBACK_URL") || captureURL;
  const apiKey = readEnvText("AIVOCLAW_ANALYTICS_API_KEY");
  const requestTimeoutMs = readEnvPositiveInt("AIVOCLAW_ANALYTICS_REQUEST_TIMEOUT_MS", 8000);
  const retryDelaysMs = readEnvRetryDelays("AIVOCLAW_ANALYTICS_RETRY_DELAYS_MS", [0, 500, 1500]);
...
function getQqbotPackageSource() {
  ...
  const explicitSource = readEnvText("AIVOCLAW_QQBOT_PACKAGE_SOURCE");
  if (explicitSource) {
    log(`使用 AIVOCLAW_QQBOT_PACKAGE_SOURCE 指定来源: ${explicitSource}`);
    return {
      source: explicitSource,
  ...
  if (!latestVersion) {
    die(`无法从 npm 获取 ${QQBOT_PACKAGE_NAME} 最新版本（检查网络或设置 AIVOCLAW_QQBOT_PACKAGE_SOURCE 手动指定）`);
  }
...
function getDingtalkConnectorPackageSource() {
  // 显式覆盖（调试 / 私有 tgz / 本地 file: 逃生舱）
  const explicitSource = readEnvText("AIVOCLAW_DINGTALK_CONNECTOR_PACKAGE_SOURCE");
  if (explicitSource) {
    log(`使用 AIVOCLAW_DINGTALK_CONNECTOR_PACKAGE_SOURCE 指定来源: ${explicitSource}`);
    return {
      source: explicitSource,
  ...
  if (!latestVersion) {
    die(`无法从 npm 获取 ${DINGTALK_CONNECTOR_PACKAGE_NAME} 最新版本（检查网络或设置 AIVOCLAW_DINGTALK_CONNECTOR_PACKAGE_SOURCE 手动指定）`);
  }
...
function getWecomPluginPackageSource() {
  // 显式覆盖（调试 / 私有 tgz / 本地 file: 逃生舱）
  const explicitSource = readEnvText("AIVOCLAW_WECOM_PLUGIN_PACKAGE_SOURCE");
  if (explicitSource) {
    log(`使用 AIVOCLAW_WECOM_PLUGIN_PACKAGE_SOURCE 指定来源: ${explicitSource}`);
    return {
      source: explicitSource,
  ...
  if (!latestVersion) {
    die(`无法从 npm 获取 ${WECOM_PLUGIN_PACKAGE_NAME} 最新版本（检查网络或设置 AIVOCLAW_WECOM_PLUGIN_PACKAGE_SOURCE 手动指定）`);
  }
...
// 是否保留 node-llama-cpp（默认移除；设置 AIVOCLAW_KEEP_LLAMA=true/1 可保留）
function shouldKeepLlamaPackages() {
  const raw = readEnvText("AIVOCLAW_KEEP_LLAMA").toLowerCase();
...
function pruneLlamaPackages(nmDir) {
  if (shouldKeepLlamaPackages()) {
    log("已保留 llama 依赖（AIVOCLAW_KEEP_LLAMA 已启用）");
    return;
  }
...
const BUNDLED_PLUGINS = [
  {
    id: "kimi-claw",
    localEnv: "AIVOCLAW_KIMI_CLAW_TGZ_PATH",
    urlEnv: "AIVOCLAW_KIMI_CLAW_TGZ_URL",
    refreshEnv: "AIVOCLAW_KIMI_CLAW_REFRESH",
  ...
  {
    id: "kimi-search",
    localEnv: "AIVOCLAW_KIMI_SEARCH_TGZ_PATH",
    urlEnv: "AIVOCLAW_KIMI_SEARCH_TGZ_URL",
    refreshEnv: "AIVOCLAW_KIMI_SEARCH_REFRESH",
...
// openclaw/skills 只保留 AivoClaw 产品需要的内置技能，上游新增 skill 不会自动打入。
...
async function bundleNpmPackagePlugin(plugin, gatewayDir, targetId, opts) {
  ...
  // 增量检测：版本戳匹配则跳过
  const stampPath = path.join(pluginDir, `.aivoclaw-${plugin.id}-stamp.json`);
  ...
  // 写入版本戳
  fs.writeFileSync(
    path.join(pluginDir, `.aivoclaw-${plugin.id}-stamp.json`),
...
async function bundlePlugin(plugin, gatewayDir, targetId, opts) {
  ...
  for (let attempt = 1; attempt <= 2; attempt++) {
    try {
      // Windows: 优先使用系统自带 bsdtar（不需要 --force-local），
      // 避免 Git 自带的 GNU tar 对 Windows 路径/长路径处理异常
      const isWin = process.platform === "win32";
      if (isWin) {
        // 使用 bsdtar（C:\Windows\System32\tar.exe），不需要 --force-local
        const sysTar = path.join(process.env.SystemRoot || "C:\\Windows", "System32", "tar.exe");
        const tarBin = fs.existsSync(sysTar) ? `"${sysTar}"` : "tar";
        execSync(`${tarBin} -xzf "${source.archivePath}" -C "${tmpDir}"`, { stdio: "inherit" });
      } else {
...
function pruneNodeModules(nmDir, platform) {
  ...
  // openclaw/extensions 不再整目录豁免，只保留 AivoClaw 需要的插件。
...
// ─── Step 2.6: 注入额外技能 ───
//
// 将 extraSkills/ 目录下所有含 SKILL.md 的子文件夹复制到
// gateway/node_modules/openclaw/skills/ 中，与上游内置技能并列。
// 如果目标目录已存在同名技能，会被覆盖（额外技能优先）。

function injectExtraSkills(gatewayDir) {
  if (!fs.existsSync(EXTRA_SKILLS_DIR)) {
    log("extraSkills/ 目录不存在，跳过额外技能注入");
    return;
  }

  const openclawSkillsDir = path.join(
    gatewayDir, "node_modules", "openclaw", "skills"
  );
  ...
  let entries;
  try {
    entries = fs.readdirSync(EXTRA_SKILLS_DIR, { withFileTypes: true });
  } catch (err) {
    log(`读取 extraSkills/ 失败: ${err.message}`);
    return;
  }
  ...
  for (const entry of entries) {
    if (!entry.isDirectory()) continue;
    const srcDir = path.join(EXTRA_SKILLS_DIR, entry.name);
    ...
    // 上游已有同名技能时跳过，避免覆盖
    const destDir = path.join(openclawSkillsDir, entry.name);
  ...
  log(`额外技能注入完成: ${injected} 个注入, ${skippedExists} 个跳过（上游已存在）, ${skippedNoMd} 个跳过（无 SKILL.md）`);
}
...
async function main() {
  ...
  // Step 2.6: 注入额外技能（extraSkills/ 目录下所有含 SKILL.md 的子文件夹）
  log("Step 2.6: 注入额外技能");
  injectExtraSkills(targetPaths.gatewayDir);
