...
interface SettingsIpcOptions {
  requestGatewayRestart?: (source: string) => void;
}

// 注册 Settings 相关 IPC
export function registerSettingsIpc(opts: SettingsIpcOptions = {}): void {
  // 写入配置后自动重启 gateway，避免新增 handler 遗漏重启调用
  const writeUserConfigAndRestart = (config: any, source = "settings:config-write") => {
    writeUserConfig(config);
    opts.requestGatewayRestart?.(source);
  };
  ...
  ipcMain.handle("settings:save-provider", async (_event, params) => {
    ...
    return runTrackedSettingsAction("save_provider", trackedProps, async () => {
      try {
        ...
        writeUserConfigAndRestart(config, "settings:save-provider");
        return { success: true };
      } catch (err: any) {
  ...
  ipcMain.handle("settings:delete-model", async (_event, params) => {
    const { providerKey, modelId } = params;
    return runTrackedSettingsAction("save_provider", { provider: providerKey, model: modelId, action: "delete" }, async () => {
      try {
        const config = readUserConfig();
        const providers = config?.models?.providers;
        if (!providers || !providers[providerKey]) {
          return { success: false, message: "未找到该供应商配置" };
        }

        const provEntry = providers[providerKey];
        const models: any[] = provEntry.models ?? [];
        const idx = models.findIndex((m: any) => m?.id === modelId);
        if (idx < 0) {
          return { success: false, message: "未找到该模型" };
        }
        ...
        // 如果当前选中的就是被删除的模型，需要切换
        const primary: string = config?.agents?.defaults?.model?.primary ?? "";
        if (primary === `${providerKey}/${modelId}`) {
          config.agents ??= {};
          config.agents.defaults ??= {};
          config.agents.defaults.model ??= {};
          if (models.length > 0) {
            // 切换到同 provider 下的第一个模型
            config.agents.defaults.model.primary = `${providerKey}/${models[0].id}`;
          } else {
            // 该 provider 下已无模型，清空选择
            config.agents.defaults.model.primary = "";
          }
        }

        // 如果该 provider 下已无模型，删除整个 provider 配置
        if (models.length === 0) {
          delete providers[providerKey];
        }

        writeUserConfigAndRestart(config, "settings:delete-model");
        return { success: true };
      } catch (err: any) {
        return { success: false, message: err.message || String(err) };
      }
  ...
  ipcMain.handle("settings:edit-model", async (_event, params) => {
    const { providerKey, modelId, newModelId, displayName, apiKey, baseURL, api } = params;
    return runTrackedSettingsAction("save_provider", { provider: providerKey, model: modelId, action: "edit" }, async () => {
      try {
        const config = readUserConfig();
        const providers = config?.models?.providers;
        if (!providers || !providers[providerKey]) {
        ...
        // 更新 provider 级别的配置（apiKey / baseUrl / api）
        if (apiKey !== undefined && apiKey !== "") {
          provEntry.apiKey = apiKey;
        }
        if (baseURL !== undefined) {
          provEntry.baseUrl = baseURL;
        }
        if (api !== undefined && api !== "") {
          provEntry.api = api;
        }

        // 在 models 数组中找到对应项，更新 id / name
        const models: any[] = provEntry.models ?? [];
        const idx = models.findIndex((m: any) => m?.id === modelId);
        if (idx < 0) {
          return { success: false, message: "未找到该模型" };
        ...
        const effectiveModelId = (newModelId && newModelId !== modelId) ? newModelId : modelId;
        models[idx] = {
          ...models[idx],
          id: effectiveModelId,
        ...
        // 如果 modelId 变更，同步更新 primary
        config.agents ??= {};
        config.agents.defaults ??= {};
        config.agents.defaults.model ??= {};
        const primary: string = config.agents.defaults.model.primary ?? "";
        if (primary === `${providerKey}/${modelId}` && effectiveModelId !== modelId) {
          config.agents.defaults.model.primary = `${providerKey}/${effectiveModelId}`;
        }

        writeUserConfigAndRestart(config, "settings:edit-model");
        return { success: true };
      } catch (err: any) {
        return { success: false, message: err.message || String(err) };
      }
    ...
    return runTrackedSettingsAction("save_channel", trackedProps, async () => {
      ...
      try {
        ...
        if (enabled === false) {
          ...
          writeUserConfigAndRestart(config, "settings:save-channel");
          // 禁用飞书时关闭"首配自动批准"窗口，但保留已消费标记，防止重复自动批准。
          closeFeishuFirstPairingWindow();
          return { success: true };
        }

        // 从参数或已有配置中获取凭据（纯 toggle 启用时参数不含凭据）
        const prevFeishu =
          config.channels?.feishu && typeof config.channels.feishu === "object"
            ? config.channels.feishu
            : {};
        const appId = params?.appId || prevFeishu.appId || "";
        const appSecret = params?.appSecret || prevFeishu.appSecret || "";

        // 纯 toggle 启用（无新凭据传入）且配置中已有凭据 → 跳过验证直接启用
        const isToggleOnly = !params?.appId && !params?.appSecret;
        if (isToggleOnly && prevFeishu.appId && prevFeishu.appSecret) {
      ...
      async () => {
        try {
          ...
          if (!enabled) {
            ...
            writeUserConfigAndRestart(config, "settings:save-qqbot");
            return { success: true };
          }

          // 从参数或已有配置中获取凭据（纯 toggle 启用时参数不含凭据）
          const prevQqbot = extractQqbotConfig(config);
          const appId = (typeof params?.appId === "string" ? params.appId.trim() : "") || prevQqbot.appId || "";
          const clientSecret = (typeof params?.clientSecret === "string" ? params.clientSecret.trim() : "") || prevQqbot.clientSecret || "";

          // 纯 toggle 启用且配置中已有凭据 → 跳过验证直接启用
          const isToggleOnly = !params?.appId && !params?.clientSecret;
          if (isToggleOnly && prevQqbot.appId && prevQqbot.clientSecret) {
            saveQqbotConfig(config, { ...prevQqbot, enabled: true });
            writeUserConfigAndRestart(config, "settings:save-qqbot");
            return { success: true };
          }
      ...
      async () => {
        try {
          ...
          if (!enabled) {
            ...
            writeUserConfigAndRestart(config, "settings:save-dingtalk");
            return { success: true };
          }

          // 从参数或已有配置中获取凭据（纯 toggle 启用时参数不含凭据）
          const prevDingtalk = extractDingtalkConfig(config);
          const clientId = (typeof params?.clientId === "string" ? params.clientId.trim() : "") || prevDingtalk.clientId || "";
          const clientSecret = (typeof params?.clientSecret === "string" ? params.clientSecret.trim() : "") || prevDingtalk.clientSecret || "";

          // 纯 toggle 启用且配置中已有凭据 → 跳过验证直接启用
          const isToggleOnly = !params?.clientId && !params?.clientSecret;
          if (isToggleOnly && prevDingtalk.clientId && prevDingtalk.clientSecret) {
            saveDingtalkConfig(config, { ...prevDingtalk, enabled: true });
            writeUserConfigAndRestart(config, "settings:save-dingtalk");
            return { success: true };
          }
      ...
      async () => {
        try {
          ...
          if (!enabled) {
            ...
            writeUserConfigAndRestart(config, "settings:save-wecom");
            return { success: true };
          }

          // 从参数或已有配置中获取凭据（纯 toggle 启用时参数不含凭据）
          const prevWecom = extractWecomConfig(config);
          const botId = (typeof params?.botId === "string" ? params.botId.trim() : "") || prevWecom.botId || "";
          const secret = (typeof params?.secret === "string" ? params.secret.trim() : "") || prevWecom.secret || "";

          // 纯 toggle 启用且配置中已有凭据 → 跳过验证直接启用
          const isToggleOnly = !params?.botId && !params?.secret;
          if (isToggleOnly && prevWecom.botId && prevWecom.secret) {
            saveWecomConfig(config, { ...prevWecom, enabled: true });
            writeUserConfigAndRestart(config, "settings:save-wecom");
            return { success: true };
          }
  ...
  ipcMain.handle("settings:remove-wecom-approved", async (_event, params) => {
    ...
    try {
      ...
      writeUserConfigAndRestart(config, "settings:wecom-approved");
      return { success: true };
    } catch (err: any) {
      return { success: false, message: err.message || String(err) };
    }
  ...
  ipcMain.handle("settings:add-feishu-group-allow-from", async (_event, params) => {
    ...
    try {
      ...
      writeUserConfigAndRestart(config, "settings:feishu-group");
      return { success: true };
    } catch (err: any) {
      return { success: false, message: err.message || String(err) };
    }
  ...
  ipcMain.handle("settings:remove-feishu-approved", async (_event, params) => {
    ...
    try {
      ...
      if (kind === "group") {
        ...
        writeUserConfigAndRestart(config, "settings:feishu-approved");
        return { success: true };
      }
    ...
    return runTrackedSettingsAction("save_kimi", { enabled }, async () => {
      try {
        ...
        if (enabled === false) {
          ...
          writeUserConfigAndRestart(config, "settings:save-kimi");
          return { success: true };
        }
  ...
  ipcMain.handle("settings:save-kimi-search-config", async (_event, params) => {
    ...
    return runTrackedSettingsAction("save_kimi_search", { enabled }, async () => {
      try {
        ...
        writeUserConfigAndRestart(config, "settings:save-kimi-search");
        return { success: true };
      } catch (err: any) {
        return { success: false, message: err.message || String(err) };
      }
    ...
    return runTrackedSettingsAction(
      ...
      async () => {
        try {
          ...
          writeUserConfigAndRestart(config, "settings:save-advanced");
          return { success: true };
        } catch (err: any) {
          return { success: false, message: err.message || String(err) };
        }
  ...
  ipcMain.handle("settings:restore-config-backup", async (_event, params) => {
    ...
    try {
      ...
      opts.requestGatewayRestart?.("settings:restore-backup");
      return { success: true };
    } catch (err: any) {
      return { success: false, message: err.message || String(err) };
    }
  ...
  ipcMain.handle("settings:restore-last-known-good", async () => {
    try {
      restoreLastKnownGoodConfigSnapshot();
      opts.requestGatewayRestart?.("settings:restore-lkg");
      return { success: true };
    } catch (err: any) {
      return { success: false, message: err.message || String(err) };
    }
  ...
  // 返回 AivoClaw 和 OpenClaw 版本信息
  ipcMain.handle("settings:get-about-info", async () => {
    const aivoClawVersion = app.getVersion();
    ...
    return { aivoClawVersion, openClawVersion };
  });
...
// 统一运行 openclaw CLI 子命令，复用 AivoClaw 内嵌 runtime 与网关入口。
